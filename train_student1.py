...
parser.add_argument('--direction', default=0.3, type=float, help='direction')
parser.add_argument('--variance', default=5.0, type=float, help='variance')
parser.add_argument('--num_workers', type=int, default=4, help='num_workers')
parser.add_argument('--best_teacher', type=int, default=1, help='Best teacher')
...
path = os.path.join(args.save_root + args.data_name + '_MultiTeacher_OurDiversity_' + str(args.direction)+ '_' + str(args.variance)+ '_KD1')
...
tcheckpoint = torch.load(os.path.join('results/' + args.data_name+ '_MultiTeacher_OurDiversity_'+ str(args.direction)+ '_' + str(args.variance), \
                                      'Best_MultiTeacher_model.t7'))
if args.best_teacher == 1:
    load_pretrained_model(tnet, tcheckpoint['Teacher1'])
elif args.best_teacher == 2:    
    load_pretrained_model(tnet, tcheckpoint['Teacher2'])
elif args.best_teacher == 3:    
    load_pretrained_model(tnet, tcheckpoint['Teacher3'])
elif args.best_teacher == 4:    
    load_pretrained_model(tnet, tcheckpoint['Teacher4'])
else:
    Exception('Invalid best_teacher...')
    
print ('The dataset used for training is:   '+ str(args.data_name))
print ('The type of noise used is:        '+ str(args.noise))
print ('The best teacher network is:      '+ str(args.best_teacher))
...
def train(epoch):
    ...
    for batch_idx, (img_teacher, img_student, target) in enumerate(trainloader):
        ...
        rb1_s1, rb2_s1, rb3_s1, mimic_s1, out_s1 = snet(img_student)
        ...
        conf_mat, acc, mAP, F1_score = ACC_evaluation(conf_mat, out_s1, target, NUM_CLASSES)
